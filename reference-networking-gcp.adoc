...
| \https://keyvault-production-aks.vault.azure.net | キーボールト | メタデータ処理用にS3バケットと通信するために、Azure Key Vaultからクライアントシークレットキーを取得するために使用されます。Cloud Volumes ONTAPサービスはこのコンポーネントを内部的に使用します。 | 標準モード、制限モード、およびプライベートモード。 | Cloud Volumes ONTAPサービスを使用できません。 
