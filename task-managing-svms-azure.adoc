...
ハイアベイラビリティ（HA）ペアの場合は、クラスタ内の各Storage VM（SVM）にアクセスし、各SVMのnic0で対応するIPを作成する必要があります。HA ペアに IP アドレスを割り当てる方法は、使用しているストレージプロトコルによって異なります。HAモードでは、フェイルオーバーと冗長性をサポートするようにIPアドレスが正しく設定されていることを確認する必要があります。これは、ストレージシステムの高可用性と信頼性を維持するために不可欠です。
...
+
iSCSI LIFは、iSCSIプロトコルを使用しない場合でもDNS通信に必要です。iSCSI LIF はフェイルオーバー時に移行されないため、この目的に使用されます。
...
network route create -vserver <svm-name> -destination 0.0.0.0/0 -gateway <ip-of-gateway-server>
----
. データLIFを作成この手順では、で割り当てたIPをデータLIFとして使用しlink:task-managing-svms-azure.html#ha-pair["前の手順"]ます。
+
...
. iSCSI LIFを作成してDNS通信を提供します。iSCSI LIFは、iSCSIプロトコルを使用しない場合でもDNS通信に必要です。
+
.. 次のコマンドを使用して、ノード 1 に iSCSI LIF を作成します。
+
[source, cli]
...
network route create -vserver <svm-name> -destination 0.0.0.0/0 -gateway <ip-of-gateway-server>
----
. NASデータLIFを作成この手順では、で割り当てたIPをデータLIFとして使用しlink:task-managing-svms-azure.html#ha-pair["前の手順"]ます。
+
.. 次のコマンドを使用して、ノード 1 に NAS LIF を作成します。
+
[source, cli]
