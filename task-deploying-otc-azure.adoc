...
NOTE: 中国地域では、シングルノード環境はCloud Volumes ONTAP 9.12.1 GAおよび9.13.0 GAでのみサポートされます。これらのバージョンは、Cloud Volumes ONTAPの新しいパッチおよびリリースにアップグレードできます。中国の地域に新しいCloud Volumes ONTAPバージョンを導入する場合は、NetAppサポートにお問い合わせください。これらの地域では、NetAppから直接購入したライセンスのみがサポートされています。
