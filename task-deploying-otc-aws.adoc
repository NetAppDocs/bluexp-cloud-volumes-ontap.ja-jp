...
| 資格情報を編集します | この Cloud Volumes ONTAP システムで使用する AWS クレデンシャルと Marketplace サブスクリプションを選択します。選択した認証情報を新しい AWS マーケットプレイスサブスクリプションに関連付けるには、「サブスクリプションの追加」をクリックします。サブスクリプションは、年間契約の場合と、 Cloud Volumes ONTAP の料金を 1 時間ごとに支払う場合があります。NetAppからライセンスを直接購入した場合（お客様所有のライセンスを使用（BYOL））、AWSサブスクリプションは必要ありません。https://docs.netapp.com/us-en/bluexp-setup-admin/task-adding-aws-accounts.html["BlueXPにAWSクレデンシャルを追加する方法について説明します"^]です。 
|===
+
次のビデオでは、従量課金制マーケットプレイスサブスクリプションを AWS 認証情報に関連付ける方法を示します。
