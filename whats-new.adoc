...
すべてのCloud Volumes ONTAPライセンスには、WORM（Write Once Read Many）とARP（Autonomous Ransomware Protection）の組み込みのデータ保護機能とセキュリティ機能が追加コストなしで含まれます。これは、ノードベースと容量ベースの両方のライセンスで、新規および既存のBYOLおよびPAYGO /マーケットプレイスの導入に適用されます。新しい価格モデルは、シングルノード構成やハイアベイラビリティ（HA）構成など、すべてのクラウドプロバイダと導入タイプに適用されます。

次のようなメリットがあります。

* WORMおよびARPを使用している既存のアカウントには、これらの機能に対する料金は請求されなくなります。この変更前に適用された容量使用量に対して課金されます。ワームおよびARPは、今後の請求で除外されます。
* 既存のアカウントでこれらの機能を有効にしていない場合は、追加料金なしでWORMとARPを選択できるようになりました。
* 新規アカウントの場合、WORMおよびARPの料金はすべてのCloud Volumes ONTAPオファリングから除外されます。
...
NDMPの設定および使用方法については、を参照してください https://docs.netapp.com/us-en/ontap/ndmp/index.html["NDMP 設定の概要"]。
