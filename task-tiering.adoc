...
* Azureでは、必要な権限がBlueXPに割り当てられていれば何も行う必要はありません。  to tier cold data to Azure Blob storage,詳細はこちら。。
...
+
BlueXPでは'システムの作成時にコールドデータ用のオブジェクトストアが作成されます  data tiering after implementing the requirements,接続または権限に問題がないことが必要です。その後は、ボリューム（および場合によっては、  that tiering is enabled on aggregates,アグリゲート）。
